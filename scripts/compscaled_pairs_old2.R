...
for(i in seq_along(my_grouping_names)){
        ...
        for(gi in  c("q1","q2","q3","q4","q5")){
                ...
                for(stat in c("Mean")){
                        ...
                        plot(minmax_scale(ydata1, idx = c(500,3500)), type = "l", ylim = c(0, 1),
                             xlab = "", ylab = "", xaxt = "n", lwd = 2,
                             col = my_color1)
                        
                        axis(side = 1, at = c(1, ncol(mat_1)/4,ncol(mat_1)*3/4, ncol(mat_1)), labels = c("-1kb","TSS","TTS","+1kb"))
                        
                        lines(minmax_scale(ydata2, idx = c(500,3500)), lwd = 2,col = my_color2)
                        
                        ################################################################################
                        
                        mtext(text = paste("Scaled", stat, "Coverage"), side = 2, line = 3, cex = 1.2)
                        mtext(text = paste0(my_grouping_names[i], " - ", gi), side = 3, line = 0.5, cex = 1.2)
...
write.table(dfends, file = gsub("pdf","txt",output_file), quote = FALSE, sep = "\t", row.names = FALSE)
